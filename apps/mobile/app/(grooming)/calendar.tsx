...
export default function GroomingCalendarScreen() {
  const { user } = useAuth();
  const [groomer, setGroomer] = useState<Groomer | null>(null);
  const [groomerDocId, setGroomerDocId] = useState<string | null>(null);
  ...
  async function loadData() {
    if (!user) return;
    const snap = await getDocs(query(collection(db, COLLECTIONS.GROOMERS), where('userId', '==', user.uid)));
    if (!snap.empty) {
      const g = { id: snap.docs[0].id, ...snap.docs[0].data() } as Groomer;
      setGroomer(g);
      setGroomerDocId(snap.docs[0].id);
      setAvailability(g.availability || {});
      setFee(String(g.fee || ''));
      setSlotDuration(g.slotDuration || 30);
    }
  }
  ...
  async function save() {
    if (!groomerDocId) return;
    setSaving(true);
    try {
      await updateDoc(doc(db, COLLECTIONS.GROOMERS, groomerDocId), {
        availability,
        slotDuration,
        fee: Number(fee) || 0,
      });
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F9FAFB' }}>
      <View style={{ backgroundColor: PURPLE, paddingHorizontal: 24, paddingTop: 16, paddingBottom: 20, borderBottomLeftRadius: 24, borderBottomRightRadius: 24 }}>
      ...
      <ScrollView style={{ flex: 1 }} showsVerticalScrollIndicator={false}>
        ...
        <View style={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 8 }}>
          <Text style={{ fontSize: 13, fontWeight: '600', color: '#374151', marginBottom: 6 }}>Valor desde (CLP)</Text>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderWidth: 1.5, borderColor: active ? PURPLE : '#E5E7EB',
                  }}
                >
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: isSelected ? PURPLE : '#fff',
                    borderWidth: 1, borderColor: isSelected ? PURPLE : '#E5E7EB',
                  }}
                >
                  ...
                  {hasSlots && <View style={{ width: 6, height: 6, borderRadius: 3, backgroundColor: isSelected ? 'rgba(255,255,255,0.8)' : PURPLE, marginTop: 4 }} />}
                </TouchableOpacity>
              );
            })}
          </View>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    backgroundColor: past ? '#F3F4F6' : active ? '#F3E8FF' : '#fff',
                    borderWidth: 1.5, borderColor: past ? '#E5E7EB' : active ? PURPLE : '#E5E7EB',
                    opacity: past ? 0.5 : 1,
                  }}
                >
                  <Text style={{ fontSize: 13, fontWeight: '600', color: past ? '#9CA3AF' : active ? PURPLE : '#6B7280' }}>
                    {past ? '⛔' : active ? '✅' : '○'} {slot}
                  </Text>
                </TouchableOpacity>
              );
        ...
        <View style={{ paddingHorizontal: 24, marginTop: 24 }}>
          ...
          <View style={{ flexDirection: 'row', gap: 10 }}>
            <TouchableOpacity
              ...
              style={{ flex: 1, borderWidth: 1.5, borderColor: PURPLE, borderRadius: 12, paddingVertical: 12, alignItems: 'center', backgroundColor: '#FAF5FF' }}
            >
              <Text style={{ color: PURPLE, fontWeight: '700', fontSize: 13 }}>📆 Toda la semana</Text>
            </TouchableOpacity>
            <TouchableOpacity
              onPress={() => applyToRange(MONTH_DAYS, 'todo el mes')}
              style={{ flex: 1, borderWidth: 1.5, borderColor: PURPLE, borderRadius: 12, paddingVertical: 12, alignItems: 'center', backgroundColor: '#FAF5FF' }}
            >
              <Text style={{ color: PURPLE, fontWeight: '700', fontSize: 13 }}>🗓️ Todo el mes</Text>
            </TouchableOpacity>
          </View>
        </View>
        ...
        <TouchableOpacity
          onPress={save}
          disabled={saving}
          style={{ backgroundColor: PURPLE, borderRadius: 16, paddingVertical: 16, alignItems: 'center', margin: 24, opacity: saving ? 0.7 : 1 }}
        >
