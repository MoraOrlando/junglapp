      ...
      {petsLookingForPartner.length === 0 ? (
        ...
        <ScrollView className="flex-1 px-6 mt-6">
          ...
          <View className="gap-3">
            {petsLookingForPartner.map((pet) => (
              ...
              >
                {pet.photos?.[0] ? (
                  <Image source={{ uri: pet.photos[0] }} style={{ width: 56, height: 56, borderRadius: 16 }} />
                ) : (
        ...
        <View className="flex-1 px-6 mt-6">
          <View className="bg-white rounded-3xl shadow-lg border border-gray-100 overflow-hidden mb-6">
            <View style={{ height: 240, backgroundColor: '#fff7ed', alignItems: 'center', justifyContent: 'center' }}>
              {candidate.photos?.[0] ? (
                <Image source={{ uri: candidate.photos[0] }} style={{ width: '100%', height: '100%' }} resizeMode="cover" />
              ) : (
