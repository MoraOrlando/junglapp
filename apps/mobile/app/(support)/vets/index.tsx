...
export default function VetsAdminScreen() {
  ...
  const [kind, setKind] = useState<Kind>(kindParam === 'clinic' ? 'clinic' : kindParam === 'vet' ? 'vet' : 'all');
  ...
  function isClinicOf(v: Veterinarian) { return (v as any).isClinic ?? false; }

  const byKind = kind === 'all' ? vets : vets.filter((v) => (kind === 'clinic') === isClinicOf(v));
  const filtered = filter === 'all' ? byKind : byKind.filter((v) => v.status === filter);
  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F5F3FF' }}>
      <View style={{ paddingHorizontal: 20, paddingTop: 16, paddingBottom: 8 }}>
        <Text style={{ fontSize: 20, fontWeight: '800', color: PURPLE, marginBottom: 12 }}>
          {kind === 'clinic' ? '🏥' : kind === 'vet' ? '🩺' : '🩺🏥'} {kind === 'clinic' ? 'Veterinarias' : kind === 'vet' ? 'Veterinarios' : 'Veterinarios y Veterinarias'} ({byKind.length})
        </Text>
        <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 8 }}>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {([['all', 'Todos'], ['vet', '🩺 Veterinarios'], ['clinic', '🏥 Veterinarias']] as const).map(([k, label]) => (
              <TouchableOpacity key={k} onPress={() => setKind(k)} style={{ backgroundColor: kind === k ? PURPLE : '#fff', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: kind === k ? PURPLE : '#E5E7EB' }}>
                <Text style={{ color: kind === k ? '#fff' : '#374151', fontWeight: '600', fontSize: 13 }}>{label}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
        <ScrollView horizontal showsHorizontalScrollIndicator={false}>
          <View style={{ flexDirection: 'row', gap: 8 }}>
            {(['all', 'pending', 'approved', 'rejected'] as const).map((f) => (
              <TouchableOpacity key={f} onPress={() => setFilter(f)} style={{ backgroundColor: filter === f ? '#EDE9FE' : '#fff', borderRadius: 20, paddingHorizontal: 14, paddingVertical: 7, borderWidth: 1, borderColor: filter === f ? PURPLE : '#E5E7EB' }}>
                <Text style={{ color: filter === f ? PURPLE : '#374151', fontWeight: '600', fontSize: 13 }}>{f === 'all' ? 'Todos' : f === 'pending' ? 'Pendientes' : f === 'approved' ? 'Aprobados' : 'Rechazados'}</Text>
              </TouchableOpacity>
            ))}
          </View>
        </ScrollView>
      ...
      <ScrollView style={{ flex: 1, paddingHorizontal: 20 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={PURPLE} />}>
        {filtered.map((v) => {
          ...
          return (
            <TouchableOpacity key={v.id} onPress={() => router.push(`/(support)/vets/${v.id}` as any)} style={{ backgroundColor: '#fff', borderRadius: 16, padding: 16, marginBottom: 12, borderWidth: 1, borderColor: '#F3F4F6' }}>
              <View style={{ flexDirection: 'row', justifyContent: 'space-between', marginBottom: 8 }}>
                <Text style={{ fontWeight: '700', color: '#1F2937', fontSize: 15, flex: 1 }}>{isClinicOf(v) ? v.name : `Dr. ${v.name}`}</Text>
