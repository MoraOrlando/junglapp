  ...
  return (
    <SafeAreaView style={{ flex: 1, backgroundColor: '#F8FAFC' }} edges={['top']}>
      <View style={{ paddingHorizontal: 24, paddingTop: 20, paddingBottom: 12 }}>
        <Text style={{ fontSize: 24, fontWeight: '800', color: '#1E293B' }}>Mensajes 💬</Text>
        <Text style={{ color: '#94A3B8', fontSize: 13, marginTop: 4 }}>Conversaciones con dueños de mascotas</Text>
      </View>
      ...
      >
        {visibleChats.length === 0 ? (
          <View style={{ alignItems: 'center', paddingVertical: 64 }}>
            ...
            <Text style={{ color: '#94A3B8', fontSize: 15, fontWeight: '600' }}>Sin conversaciones aún</Text>
          ...
          <View style={{ gap: 8, paddingBottom: 24 }}>
            {visibleChats.map((chat) => {
              ...
              return (
                <TouchableOpacity
                  key={chat.id}
                  onPress={() => router.push(`/(vet)/chat/${chat.id}` as any)}
                  activeOpacity={0.8}
                  style={{
                    ...
                    gap: 12,
                    borderWidth: 1,
                    borderColor: hasNew ? '#BFDBFE' : '#F1F5F9',
                    shadowColor: '#000',
